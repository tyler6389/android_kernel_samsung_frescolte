...
static int check_diagchar_enabled(void)
{
	int ret, enabled;
	char str[20], *buf, data;

	ret = of_parse_args_on_subcmdline("diag=",(char *)str);
	if (ret) {
		pr_err("%s: enable_diag read error\n", __func__);
		return -1;
	}

	buf = str;
	strcpy(&data, &buf[0]);
	sscanf(&data, "%d", (unsigned int *)&enabled);
...
#endif /* CONFIG_OF_SUBCMDLINE_PARSE */
